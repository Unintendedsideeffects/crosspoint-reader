...
constexpr char SettingsPageHtml[] PROGMEM =
    R"rawliteral(<!DOCTYPE html><html><head><meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0" /><title>CrossPoint Reader - Settings</title> <style>
    :root {
      --black: #000;
      --white: #fff;
      --gray-2: #222;
  ...
  </style> </head><body><nav class="nav"><a href="/">[Home]</a><a href="/files">[Files]</a><a href="/settings" class="active">[Settings]</a></nav><h1>Settings</h1><div id="message" class="message"></div><div id="settings-container"><div class="loader-container"><span class="loader"></span></div></div><div class="save-container" id="save-container" style="display:none;"><button class="save-btn" id="saveBtn" onclick="saveSettings()">Save Settings</button></div><div style="text-align:center; padding:12px 16px 20px; font-size:0.65em; color:var(--gray-5); border-top:1px solid var(--gray-7); margin:0 16px; letter-spacing:0.06em;"> CrossPoint &middot; Open Source &middot; Built for Reading </div> <script>
  let allSettings = [];
  let originalValues = {};
  let touchedFields = {};
