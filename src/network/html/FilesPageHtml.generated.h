...
constexpr char FilesPageHtml[] PROGMEM =
    R"rawliteral(<!DOCTYPE html><html><head><meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0" /><title>CrossPoint Reader - Files</title> <style>
    :root {
      --black: #000;
      --white: #fff;
      --gray-2: #222;
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .file-table tr:hover {
      background-image: repeating-linear-gradient(45deg, transparent, transparent 2px, var(--gray-9) 2px, var(--gray-9) 4px);
    }
    .epub-file {
      background-color: var(--gray-9) !important;
    }
    .epub-file:hover {
      background-image: repeating-linear-gradient(45deg, transparent, transparent 2px, var(--gray-8) 2px, var(--gray-8) 4px) !important;
    }
    .folder-row {
      background-color: var(--white) !important;
    }
    .folder-row:hover {
      background-image: repeating-linear-gradient(45deg, transparent, transparent 2px, var(--gray-9) 2px, var(--gray-9) 4px) !important;
    }
    .epub-badge {
    ...
    if (files.length === 0) {
      ...
      sortedFiles.forEach(file => {
        if (file.isDirectory) {
          ...
          fileTableContent += `<td class="actions-col"><button class="delete-btn" data-name="${encodeURIComponent(file.name)}" data-path="${encodeURIComponent(folderPath)}" data-is-folder="true" title="Delete folder">🗑️</button></td>`;
          fileTableContent += '</tr>';
        } else {
          ...
          fileTableContent += '</td>';
          fileTableContent += `<td>${escapeHtml(file.name.split('.').pop().toUpperCase())}</td>`;
          fileTableContent += `<td>${formatFileSize(file.size)}</td>`;
          fileTableContent += `<td class="actions-col"><button class="delete-btn" data-name="${encodeURIComponent(file.name)}" data-path="${encodeURIComponent(filePath)}" data-is-folder="false" title="Delete file">🗑️</button></td>`;
          fileTableContent += '</tr>';
        }
      });
      ...
      fileTable.querySelectorAll('.delete-btn').forEach(btn => {
...
function uploadFileWebSocket(file, onProgress, onComplete, onError) {
  return new Promise((resolve, reject) => {
    ...
    ws.onopen = function() {
      ...
      // Send start message with URL-encoded values to handle special characters
      ws.send(`START:${encodeURIComponent(file.name)}:${file.size}:${encodeURIComponent(currentPath)}`);
    };
