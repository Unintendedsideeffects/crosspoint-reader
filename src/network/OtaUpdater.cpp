...
bool OtaUpdater::isUpdateNewer() const {
  ...
  // Parse and validate - require exactly 3 segments and no extra characters
  int latestConsumed = 0;
  if (sscanf(latestStr, "%d.%d.%d%n", &latestMajor, &latestMinor, &latestPatch, &latestConsumed) != 3 ||
      latestStr[latestConsumed] != '\0') {
    Serial.printf("[%lu] [OTA] Failed to parse latest version: %s\n", millis(), latestVersion.c_str());
    return false;
  }

  int currentConsumed = 0;
  if (sscanf(currentStr, "%d.%d.%d%n", &currentMajor, &currentMinor, &currentPatch, &currentConsumed) != 3 ||
      currentStr[currentConsumed] != '\0') {
